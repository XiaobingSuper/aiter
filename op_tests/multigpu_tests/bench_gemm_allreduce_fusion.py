...
This follows the general workflow of `flydsl-examples-xiaobing/bench_gemm.py`,
but benchmarks the fused FlyDSL GEMM+all-reduce path against the unfused
...
from aiter.ops.flydsl import flydsl_hgemm, flydsl_hgemm_ar
...
from aiter.ops.flydsl.hgemm_qr import (
    experimental_hgemm_qr_enabled,
    supports_hgemm_qr,
)
from aiter.ops.flydsl.kernels.hgemm_ar_params import (
    TP_HGEMM_AR_CONFIG_SELECTIONS,
    TP_HGEMM_AR_CONFIG_VARIANTS,
    flydsl_tp_hgemm_ar_kernel_name,
)
from aiter.ops.flydsl.kernels.splitk_hgemm_ar import compile_hgemm_ar_kernel
...
BENCHMARK_MODES = ("eager", "graph")
QUICK_REDUCE_CHOICES = ("FP", "FP8", "INT6", "INT4", "NONE")
...
INT4_DIRECT_QR_MAX_ERR_RATIO = 0.01
...
HELP_EPILOG = """Examples:
  Tune a CSV of GEMM+AR shapes on 2 GPUs:
    python3 op_tests/multigpu_tests/bench_gemm_allreduce_fusion.py \\
      --tp 2 \\
      --input-csv aiter/configs/model_configs/tp_gemm_ar_smoke_untuned_gemm_bf16.csv

  Tune a single shape:
    python3 op_tests/multigpu_tests/bench_gemm_allreduce_fusion.py \\
      --tp 2 --m 32 --n 1024 --k 7168 --dtype bf16

  Graph benchmark with INT4 quick-reduce:
    python3 op_tests/multigpu_tests/bench_gemm_allreduce_fusion.py \\
      --tp 4 --input-csv aiter/configs/model_configs/tp_gemm_ar_smoke_untuned_gemm_bf16.csv \\
      --search-mode graph --benchmark-mode graph --quick-reduce-quantization INT4
"""
...
def _configure_quick_reduce(quantization: Optional[str]) -> None:
    if quantization is None:
        return
    normalized = str(quantization).strip().upper()
    if normalized not in QUICK_REDUCE_CHOICES:
        raise ValueError(
            f"unsupported quick reduce quantization: {quantization!r}; "
            f"expected one of {QUICK_REDUCE_CHOICES}"
        )
    os.environ["AITER_QUICK_REDUCE_QUANTIZATION"] = normalized
...
def _run_direct_qr_reference(
    ...
    if qr_comm is None or getattr(qr_comm, "disabled", True):
        raise RuntimeError("QuickReduce communicator is unavailable")
    qr_comm.quick_all_reduce(out, out=out)
    return out


def _uses_direct_qr_baseline(
    config: dict[str, Any], quick_reduce_quantization: Optional[str]
) -> bool:
    if str(quick_reduce_quantization or "").strip().upper() != "INT4":
        return False
    if not experimental_hgemm_qr_enabled():
        return False
    config = _normalize_config(config)
    return supports_hgemm_qr(
        tile_m=config["tile_m"],
        tile_n=config["tile_n"],
        tile_k=config["tile_k"],
        split_k=config["split_k"],
...
def _filter_int4_fused_configs(
    valid_configs: list[dict[str, Any]],
    quick_reduce_quantization: Optional[str],
) -> list[dict[str, Any]]:
    if str(quick_reduce_quantization or "").strip().upper() != "INT4":
    ...
    qr_configs = [
        ...
        if _uses_direct_qr_baseline(config, quick_reduce_quantization)
    ]
    return qr_configs or valid_configs
...
def _capture_graph_runner(
    fn: Callable[[], torch.Tensor],
    *,
    device_group,
    device: torch.device,
) -> Callable[[], torch.Tensor]:
    # Warm once before capture so compile/init cost does not pollute replay timing.
    out = fn()
    torch.cuda.synchronize(device)
    _sync_group(device_group, device)

    graph = torch.cuda.CUDAGraph()
    with get_tp_group().graph_capture() as graph_capture_context:
...
@contextmanager
def _prepared_benchmark_runner(
    fn: Callable[[], torch.Tensor],
    *,
    mode: str,
    device_group,
    device: torch.device,
) -> Iterator[Callable[[], torch.Tensor]]:
    if mode == "eager":
        yield fn
        return
    if mode != "graph":
        raise ValueError(f"unsupported benchmark mode: {mode!r}")
    yield _capture_graph_runner(fn, device_group=device_group, device=device)
...
def _default_output_csv(
    input_csv: str,
    *,
    search_mode: str = "eager",
    benchmark_mode: str = "eager",
    quick_reduce_quantization: Optional[str] = None,
) -> str:
    path = Path(input_csv)
    name = path.name
    if name.endswith("_untuned_gemm_bf16.csv"):
        stem = name.replace("_untuned_gemm_bf16.csv", "_bf16_tuned_tp_gemm_ar")
    else:
        stem = f"{path.stem}_tuned_tp_gemm_ar"

    suffixes: list[str] = []
    if search_mode != "eager" or benchmark_mode != "eager":
        if search_mode == benchmark_mode:
            suffixes.append(search_mode)
        else:
            suffixes.append(f"search_{search_mode}")
            suffixes.append(f"bench_{benchmark_mode}")
    if quick_reduce_quantization and quick_reduce_quantization != "NONE":
        suffixes.append(quick_reduce_quantization.lower())

    out_name = stem
    if suffixes:
        out_name += "_" + "_".join(suffixes)
    out_name += ".csv"
...
def _worker(
    ...
    try:
        _configure_quick_reduce(quick_reduce_quantization)
        ...
        if rank_id == 0:
            print(
                f"[rank0] search_mode={search_mode}, benchmark_mode={benchmark_mode}, "
                f"quick_reduce_quantization={quick_reduce_quantization}",
                flush=True,
            )
        ...
        for idx, args in enumerate(shape_args):
            ...
            try:
                ...
                full_ref_out = _run_reference(a, b)
                tol = _get_tol(args.k, args.world_size)

                valid_configs = _get_supported_configs(args, show_progress=rank_id == 0)
                filtered_configs = _filter_int4_fused_configs(
                    valid_configs, quick_reduce_quantization
                )
                if rank_id == 0 and len(filtered_configs) != len(valid_configs):
                    print(
                        f"[rank0] filtered INT4 fused candidates for M={args.m}, N={args.n}, K={args.k}: "
                        f"{len(valid_configs)} -> {len(filtered_configs)}",
                        flush=True,
                    )
                ...
                try:
                    for idx_cfg, config in enumerate(valid_configs, start=1):
                        ...
                        with _prepared_benchmark_runner(
                            ...
                            candidate_ref_out = full_ref_out
                            use_direct_qr_baseline = _uses_direct_qr_baseline(
                                config, quick_reduce_quantization
                            )
                            if use_direct_qr_baseline:
                                candidate_ref_out = _run_direct_qr_reference(a, b, config)
                            if use_direct_qr_baseline:
                                candidate_err_ratio = checkAllclose(
                                    candidate_ref_out,
                                    out,
                                    rtol=tol,
                                    atol=tol,
                                ...
                                is_ok = (
                                    float(candidate_err_ratio)
                                    <= INT4_DIRECT_QR_MAX_ERR_RATIO
                                )
                            else:
                                is_ok = torch.allclose(
                                    out, candidate_ref_out, atol=tol, rtol=tol
                                )
                            ok_tensor = torch.tensor(
                                1 if is_ok else 0, device=device, dtype=torch.int32
                            )
                            dist.all_reduce(
                                ok_tensor,
                ...
                ref_fn: Callable[[], torch.Tensor] = lambda: _run_reference(a, b)
                if _uses_direct_qr_baseline(best_config, quick_reduce_quantization):
                    ref_fn = lambda cfg=best_config: _run_direct_qr_reference(a, b, cfg)
                with _prepared_benchmark_runner(
                    ref_fn,
                    mode=benchmark_mode,
                    device_group=tp_group.device_group,
                    ...
                    benchmark_ref_out, ref_us = _measure_runtime_us(
                        ref_runner,
                        warmup=warmup,
                        niters=niters,
                        device_group=tp_group.device_group,
                ...
                del fused_out, benchmark_ref_out, full_ref_out, a, b
...
def tune_shapes_from_csv(
    ...
    *,
    ...
    shape_args = []
    for row in rows:
        if _as_bool(row.get("bias", False)) or _as_bool(row.get("scaleAB", False)):
            continue
        shape_args.append(
            Args(
    ...
    with output_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for result in shape_pbar:
            args = result["shape"]
            shape_pbar.set_postfix_str(
                f"tp={args.world_size},M={args.m},N={args.n},K={args.k}"
            )
            if result["error"] is not None:
                print(
                    f"skip shape tp={args.world_size}, M={args.m}, N={args.n}, K={args.k}: "
                    f"{result['error']}",
                    flush=True,
                )
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Measurement mode used during candidate search.",
    )
    parser.add_argument(
        "--benchmark-mode",
        type=str,
        default="eager",
        choices=BENCHMARK_MODES,
        help="Measurement mode used for final fused-vs-reference benchmarking.",
    )
    parser.add_argument(
        "--quick-reduce-quantization",
        type=str.upper,
        choices=QUICK_REDUCE_CHOICES,
        help="Override AITER_QUICK_REDUCE_QUANTIZATION in tuning workers.",
    )
    parser.add_argument(
        "--input-csv",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Latest FlyDSL split_k>1 path always forces cross-device atomic."
        ),
    )
    ...
    if cli_args.input_csv:
        output_csv = cli_args.output_csv or _default_output_csv(
            cli_args.input_csv,
            search_mode=cli_args.search_mode,
            benchmark_mode=cli_args.benchmark_mode,
            quick_reduce_quantization=cli_args.quick_reduce_quantization,
        )
