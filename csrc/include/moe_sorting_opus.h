    ...
    : MoeSortingKernel<MoeSortingProblemEx<opus::index_t, float, 1, true, false, false>>
{
    ...
    static constexpr int kMaxBitsetWords = 2048; // 8 KiB bitset + 8 KiB prefix counts.
    ...
    OPUS_H static bool IsPreferred(const Hargs& h)
    {
        if(h.num_experts <= 0 || h.num_experts > kMaxExperts || h.topk <= 0 ||
           h.topk > h.num_experts || h.topk >= 256 || h.tokens <= 0 ||
           h.tokens > kMaxRoutes / h.topk || h.unit_size <= 0)
            return false;
    ...
    OPUS_H static auto GetSmemSize(const Hargs& h)
    {
        ...
        return (h.num_experts * (2 * words + 2) + 1) * sizeof(opus::index_t);
    }
    ...
    OPUS_D void operator()(Kargs k) const
    {
        ...
        const int words      = (routes + 31) / 32;
        auto* prefix         = bits + k.num_experts * words;
        ...
        if(tid < k.num_experts)
        {
            ...
            {
                prefix[word * k.num_experts + tid] = count;
        ...
        for(int i = tid; i < routes; i += kBlockSize)
        {
            ...
            if(static_cast<unsigned>(e) < k.num_experts)
            {
                // Reuse the word-prefix count instead of rescanning earlier words per route.
                int pos = offsets[e] + prefix[(i / 32) * k.num_experts + e];
